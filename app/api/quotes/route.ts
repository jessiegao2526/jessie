...
export async function GET(request:Request){
  ...
  const quotes=await Promise.all(secids.map(async secid=>{
    ...
    const r=await fetch(url,{headers:{"User-Agent":"Mozilla/5.0"},cache:"no-store"}); const j:any=await r.json(); const d=j.data;
  ...
  return Response.json({quotes},{headers:{"Cache-Control":"no-store, max-age=0"}});
}
