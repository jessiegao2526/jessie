...
export default function Home(){
  ...
  async function generateSpeech(){if(!speechSelected)return;setSpeechLoading(true);setSpeechError("");try{const d=await fetchQuote(speechSelected.secid,true);const price=Number(d.f43)/100;const market=speechSelected.market;const profile=await fetch(`/api/profile?code=${market}${d.f57}`).then(r=>r.ok?r.json():null).catch(()=>null);const concepts=String(d.f129||"").split(",").filter((x:string)=>x&&!x.includes("板块")&&!x.includes("融资")&&!x.includes("转融券")).slice(0,2);const segments=(profile?.segments||[]) as {name:string;revenueRatio:number;grossMargin:number}[];const products=(profile?.products||[]) as {name:string;revenueRatio:number;grossMargin:number}[];const top=segments[0];const productNames=products.filter(x=>x.name&&x.name!=="其他").slice(0,2).map(x=>x.name);const quality=[...products].filter(x=>x.name&&x.name!=="其他"&&x.grossMargin>0).sort((a,b)=>b.grossMargin-a.grossMargin)[0];const lines:string[]=[];if(top)lines.push(`${top.name}贡献${(top.revenueRatio*100).toFixed(1)}%营收，是当前核心主业`);else lines.push(`核心业务聚焦${profile?.industry||d.f127||"所属行业"}`);if(productNames.length)lines.push(`${productNames.join("、")}构成主要产品线`);if(quality)lines.push(`${quality.name}毛利率约${(quality.grossMargin*100).toFixed(1)}%，盈利质量更优`);if(concepts.length)lines.push(`${concepts.join("、")}是主要市场映射`);lines.push(`现价${price.toFixed(2)}元，建仓一成，止损${(price*.85).toFixed(2)}元`);setSpeech(`${d.f58}（${d.f57}）\n${lines.slice(0,5).map((x,i)=>`${i+1}. ${x}`).join("\n")}`);setSpeechStock({secid:speechSelected.secid,code:d.f57,name:d.f58,market,price});setSpeechCost(price.toFixed(2));setSpeechQuery(`${d.f58}  ${d.f57}`)}catch{setSpeechError("暂时无法取得该股票资料，请稍后重试")}finally{setSpeechLoading(false)}}
  ...
  return <main>
    ...
    <section className="generator"><div className="generatorCopy"><p className="eyebrow">XINHUIYING SCRIPT</p><h1>鑫汇盈话术生成器</h1><p>输入股票名称或代码，即刻生成简洁的买入逻辑与参考止损价。</p></div><div className="generatorPanel">
      ...
      {speechError&&<p className="formError">{speechError}</p>}{speech&&<div className="speechResult"><span>智能逻辑已生成</span><p>{speech}</p><div className="speechButtons"><button onClick={copySpeech}>复制话术</button><button className="addPosition" onClick={openSpeechAdd}>＋ 加入持仓</button></div></div>}
      <small className="generatorNote">优先读取最新财报主营构成，再结合产品毛利与概念映射 · 止损价按现价 85% 计算</small>
    </div></section>
