...
export default function Home(){
  ...
  async function generateSpeech(){if(!speechSelected)return;setSpeechLoading(true);setSpeechError("");try{const d=await fetchQuote(speechSelected.secid,true);const price=Number(d.f43)/100;const market=speechSelected.market;const profileRequest=fetch(`/api/profile?code=${market}${d.f57}`).then(r=>r.ok?r.json():null).catch(()=>null);const profile=await profileRequest;const concepts=String(d.f129||"").split(",").filter((x:string)=>x&&!x.includes("板块")&&!x.includes("融资")).slice(0,2);const rawBusiness=String(profile?.businessScope||"").replace(/^(许可项目|一般项目)[:：]?/,"").split(/[;；。]/)[0];const business=(rawBusiness||profile?.industry||d.f127||"核心业务").slice(0,16);const driver=concepts.length?`主营${business}，叠加${concepts.join("、")}概念`:`聚焦${business}，行业景气度值得关注`;setSpeech(`${d.f58}（${d.f57}）：${driver}；建仓一成，止损价建议设为${(price*.85).toFixed(2)}元左右`);setSpeechStock({secid:speechSelected.secid,code:d.f57,name:d.f58,market,price});setSpeechCost(price.toFixed(2));setSpeechQuery(`${d.f58}  ${d.f57}`)}catch{setSpeechError("暂时无法取得该股票资料，请稍后重试")}finally{setSpeechLoading(false)}}
  ...
  return <main>
    ...
    <section className="generator"><div className="generatorCopy"><p className="eyebrow">XINHUIYING SCRIPT</p><h1>鑫汇盈话术生成器</h1><p>输入股票名称或代码，即刻生成简洁的买入逻辑与参考止损价。</p></div><div className="generatorPanel">
      ...
      <small className="generatorNote">综合主营业务、所属行业与热门概念 · 止损参考价按实时现价的 85% 计算</small>
    </div></section>
